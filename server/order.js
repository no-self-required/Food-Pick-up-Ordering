...
module.exports = (db) => {
  ...
  const orderCheckout = async function (cart) {
    return (
      db
        .query(
          `INSERT INTO orders (phone, total_price, order_status)
      VALUES (${cart.phoneNumber}, ${cart.total}, 1) RETURNING *`
        ...
        // DISABLE TWILIO FUNCTIONALITY (app not active)
        // .then((id) => {
        //   client.messages
        //     .create({
        ...
        //       from: "+17147092156",
        //       to: `+17052415272`, //restaurant number
        //     })
        //     .then((message) =>
        //       res.send(`The message to: ${message.to} was sent `)
        //     );
        // })
        .catch((err) => {
          console.log(err);
